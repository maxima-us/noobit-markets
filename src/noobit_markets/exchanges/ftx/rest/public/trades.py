...
def parse_request(
    ...
    payload: _ParsedReq = {
        "market_name": valid_request.symbols_resp.asset_pairs.get(
...
def parse_result(
    result_data: FtxResponseTrades, symbol: ntypes.SYMBOL
) -> T_PublicTradesParsedRes:
...
def _single_trade(
    data: FtxResponseItemTrades, symbol: ntypes.SYMBOL
) -> T_PublicTradesParsedItem:
...
async def get_trades_ftx(
    ...
    *,
    ...
    result_content = await get_result_content_from_req(
        client, "GET", req_url, valid_ftx_req.value, headers
    )
