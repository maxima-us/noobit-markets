...
def parse_result_data_orders(
        ...
        asset_mapping: ntypes.SYMBOL_FROM_EXCHANGE
    ) -> typing.Tuple[pmap]:

    parsed = [_single_order(item) for item in result_data.orders]

    return tuple(parsed)


def _single_order(item: BinanceResponseItemOrders, asset_mapping) -> pmap:

    parsed = {
        "orderID": asset_mapping[item.orderId],
        "symbol":item.symbol,
        "currency": None,
