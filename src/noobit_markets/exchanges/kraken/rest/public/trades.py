...
_TradesItem = typing.Tuple[
    Decimal, Decimal, Decimal, Literal["b", "s"], Literal["m", "l"], Any
]
...
def make_kraken_model_trades(
    ...
    model = pydantic.create_model("KrakenResponseTrades", **kwargs)  # type: ignore

    return model


def parse_result(
    result_data: typing.Tuple[_TradesItem, ...], symbol: ntypes.SYMBOL
) -> T_PublicTradesParsedRes:
...
def _single_trade(data: _TradesItem, symbol: ntypes.SYMBOL) -> T_PublicTradesParsedItem:
...
async def get_trades_kraken(
    ...
    *,
    ...
    valid_noobit_req = _validate_data(
        NoobitRequestTrades,
        pmap({"symbol": symbol, "symbols_resp": symbols_resp, "since": since}),
    )
    ...
    valid_result_content = _validate_data(
        make_kraken_model_trades(symbol, symbol_to_exchange), result_content.value
    )
