...
def parse_request(
    valid_request: NoobitRequestSpread, symbol_to_exchange: ntypes.SYMBOL_TO_EXCHANGE
) -> _ParsedReq:
...
def make_kraken_model_spread(
    ...
    model = pydantic.create_model("KrakenResponseSpread", **kwargs)  # type: ignore

    return model


def parse_result(
    result_data_spread: typing.Tuple[_SpreadItem, ...], symbol: ntypes.SYMBOL
...
def _single(data: _SpreadItem, symbol: ntypes.SYMBOL) -> T_SpreadParsedRes:
...
async def get_spread_kraken(
    ...
    *,
    ...
    valid_noobit_req = _validate_data(
        NoobitRequestSpread, pmap({"symbol": symbol, "symbols_resp": symbols_resp})
    )
    ...
    valid_result_content = _validate_data(
        make_kraken_model_spread(symbol, symbol_to_exchange), result_content.value
    )
