...
def parse_result(
    ...
    parsed = {
        asset_from_exchange(asset): amount
        for asset, amount in result_data.items()
        if not asset == "KFEE" and amount > Decimal(0) and "." not in asset
    }
    return pmap(parsed)
...
async def get_balances_kraken(
    ...
    *,
    ...
    parsed_result_data = parse_result(
        valid_result_content.value.balances, asset_from_exchange
    )
